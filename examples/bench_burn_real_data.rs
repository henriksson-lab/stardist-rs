...
mod bench {
    ...
    pub fn main() -> Result<(), Box<dyn Error>> {
        ...
        if args.len() < 3 || (args[1] != "2d" && args[1] != "3d") {
            eprintln!(
                "usage: cargo run --release --features burn --example bench_burn_real_data -- <2d|3d> <python_npz>"
            );
            std::process::exit(2);
        }

        match args[1].as_str() {
            "2d" => run_2d(Path::new(&args[2]))?,
            "3d" => run_3d(Path::new(&args[2]))?,
            _ => unreachable!(),
        }
        Ok(())
    }

    fn run_2d(npz_path: &Path) -> Result<(), Box<dyn Error>> {
        ...
        let model = stardist_rs::model::burn::StarDist2D::<B>::init(config.clone(), &device)
            .load_keras_weights(
        ...
        let tensor =
            Tensor::<B, 4>::from_data(TensorData::new(input.values.clone(), input_shape), &device);
        ...
        let prob = prob_data.as_slice::<f32>()?;
        let dist_nchw = dist_data.as_slice::<f32>()?;
        ...
        let sparse = stardist.predict_sparse(
            ...
            None,
            2,
            |x, x_shape, axes| predict_direct_2d(&model, &config, x, x_shape, axes),
        )?;
    ...
    fn run_3d(npz_path: &Path) -> Result<(), Box<dyn Error>> {
        let mut npz = NpzReader::new(File::open(npz_path)?)?;
        ...
        let config = stardist.config.clone();
        let model = stardist_rs::model::burn::StarDist3D::<B>::init(config.clone(), &device)
            .load_keras_weights(
        ...
        let load_rss_kib = max_rss_kib();
        let tensor =
            Tensor::<B, 5>::from_data(TensorData::new(input.values.clone(), input_shape), &device);
        let started = Instant::now();
        let outputs = model.forward(tensor);
        let inference_seconds = started.elapsed().as_secs_f64();
        let prob_data = outputs.prob.into_data();
        let dist_data = outputs.dist.into_data();
        let prob = prob_data.as_slice::<f32>()?;
        let dist_ncdhw = dist_data.as_slice::<f32>()?;
        ...
        let sparse = stardist.predict_sparse(
            ...
            None,
            2,
            |x, x_shape, axes| predict_direct_3d(&model, &config, x, x_shape, axes),
        )?;
        let predict_sparse_seconds = started.elapsed().as_secs_f64();
        let started = Instant::now();
        let instances = stardist._instances_from_prediction(
        ...
        print_report(
            ...
            &[
                ("raw_prob", compare_f32(prob, &python_raw_prob.values)),
                ("raw_dist", compare_f32(dist_ncdhw, &python_raw_dist.values)),
                (
                    "sparse_prob",
                    compare_f32(&sparse.prob, &python_sparse_prob.values),
                ),
    ...
    fn predict_direct_2d(
        model: &stardist_rs::model::burn::StarDist2D<B>,
        ...
        let device = Default::default();
        let input =
            Tensor::<B, 4>::from_data(TensorData::new(nchw, [1, 1, height, width]), &device);
    ...
    fn predict_direct_3d(
        model: &stardist_rs::model::burn::StarDist3D<B>,
        ...
        let input = Tensor::<B, 5>::from_data(
            TensorData::new(ncdhw, [1, 1, depth, height, width]),
            &device,
        );
        let outputs = model.forward(input);
        let prob_data = outputs.prob.into_data();
        let dist_data = outputs.dist.into_data();
        let prob_ncdhw = prob_data
            .as_slice::<f32>()
            .map_err(|_| stardist_rs::StarDistPredictError::OutputShapeMismatch)?;
        let dist_ncdhw = dist_data
            .as_slice::<f32>()
            .map_err(|_| stardist_rs::StarDistPredictError::OutputShapeMismatch)?;
...
#[cfg(not(all(feature = "burn", feature = "hdf5")))]
fn main() {
    eprintln!("bench_burn_real_data requires --features burn,hdf5");
    std::process::exit(2);
}
