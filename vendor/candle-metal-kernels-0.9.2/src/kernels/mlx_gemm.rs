...
pub fn call_mlx_gemm(
    ...
    let pipeline = match kernels.load_pipeline_with_constants(device, Source::Gemm, name, constants)
    {
        Ok(pipeline) => pipeline,
        Err(err) if dtype == GemmDType::F32 => {
            return call_portable_gemm_f32(
                device,
                ep,
                kernels,
                (b, m, n, k),
            ...
            .map_err(|fallback_err| {
                MetalKernelError::FailedToCreatePipeline(format!(
                    "mlx gemm failed: {err}; portable fallback failed: {fallback_err}"
                ))
            });
        }
...
fn call_portable_gemm_f32(
    ...
    impl EncoderParam for PortableGemmParams {
        fn set_param(encoder: &ComputeCommandEncoder, position: usize, data: Self) {
            encoder.set_bytes(position, &data);
        }
    }

    let lhs_stride_b = if lhs_stride.len() > 2 {
        lhs_stride[lhs_stride.len() - 3]
    } else {
        m * k
    };
    let rhs_stride_b = if rhs_stride.len() > 2 {
        rhs_stride[rhs_stride.len() - 3]
    } else {
        k * n
    };
    let params = PortableGemmParams {
        m: m as u32,
        n: n as u32,
        k: k as u32,
        batch: b as u32,
        lhs_stride_b: lhs_stride_b as u32,
        lhs_stride_m: lhs_stride[lhs_stride.len() - 2] as u32,
        lhs_stride_k: lhs_stride[lhs_stride.len() - 1] as u32,
        rhs_stride_b: rhs_stride_b as u32,
        rhs_stride_k: rhs_stride[rhs_stride.len() - 2] as u32,
        rhs_stride_n: rhs_stride[rhs_stride.len() - 1] as u32,
    };

    let pipeline = kernels.load_pipeline(device, Source::PortableGemm, "portable_gemm_f32")?;
    let encoder = ep.encoder();
    let encoder: &ComputeCommandEncoder = encoder.as_ref();
    encoder.set_compute_pipeline_state(&pipeline);
