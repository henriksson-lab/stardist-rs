...
pub fn call_im2col3d_strided(
    ...
    shape: &[usize],
    strides: &[usize],
    (d_k, h_k, w_k, stride, padding, dilation): (usize, usize, usize, usize, usize, usize),
    input: BufferOffset,
    output: &Buffer,
) -> Result<(), MetalKernelError> {
    let pipeline = kernels.load_pipeline(device, Source::Conv, name)?;
    ...
    let dst_el = shape[0] * d_out * h_out * w_out * shape[1] * d_k * h_k * w_k;
    ...
    set_params!(
        encoder,
        (
            dst_el, d_out, h_out, w_out, d_k, h_k, w_k, stride, padding, dilation, shape, strides,
            &input, output
        )
    );
    encoder.use_resource(input.buffer, MTLResourceUsage::Read);
