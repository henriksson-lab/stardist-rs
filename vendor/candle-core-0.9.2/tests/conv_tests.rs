...
fn conv3d_reference(
    ...
    let out_d = (i_d + 2 * padding - (k_d - 1) - 1) / stride + 1;
    let out_h = (i_h + 2 * padding - (k_h - 1) - 1) / stride + 1;
    let out_w = (i_w + 2 * padding - (k_w - 1) - 1) / stride + 1;
                ...
                for oh in 0..out_h {
                    for ow in 0..out_w {
                        ...
                        for ic in 0..c_in {
                            for kd in 0..k_d {
                                let id = od * stride + kd;
                                if id < padding || id >= i_d + padding {
                                    continue;
                                }
                                let id = id - padding;
                                for kh in 0..k_h {
                                    let ih = oh * stride + kh;
                                    if ih < padding || ih >= i_h + padding {
                                        continue;
                                    }
                                    let ih = ih - padding;
                                    for kw in 0..k_w {
                                        let iw = ow * stride + kw;
                                        if iw < padding || iw >= i_w + padding {
                                            continue;
                                        }
                                        let iw = iw - padding;
                                        let input_idx =
                        ...
                        let out_idx =
                            ((((b * c_out + oc) * out_d + od) * out_h + oh) * out_w) + ow;
                        out[out_idx] = sum;
                    }
                }
            }
...
fn conv3d_cpu_plan_cases() -> Result<()> {
    conv3d_plan_cases(&Device::Cpu)
}

fn conv3d_plan_cases(dev: &Device) -> Result<()> {
    let input = (0..8).map(|v| v as f32).collect::<Vec<_>>();
    let kernel = vec![2.];
    let res = Tensor::new(input.clone(), dev)?
        .reshape((1, 1, 2, 2, 2))?
        .conv3d(&Tensor::new(kernel.clone(), dev)?.reshape((1, 1, 1, 1, 1))?, 0, 1, 1, 1)?;
    assert_eq!(res.dims(), &[1, 1, 2, 2, 2]);
    assert_eq!(
        test_utils::to_vec1_round(&res.flatten_all()?, 4)?,
        conv3d_reference(&input, &kernel, (1, 1, 2, 2, 2), (1, 1, 1, 1, 1), 0, 1)
    );
    ...
    let res = Tensor::new(input.clone(), dev)?
        .reshape((1, 1, 3, 3, 3))?
        .conv3d(&Tensor::new(kernel.clone(), dev)?.reshape((1, 1, 3, 3, 3))?, 1, 1, 1, 1)?;
    assert_eq!(res.dims(), &[1, 1, 3, 3, 3]);
    assert_eq!(
        test_utils::to_vec1_round(&res.flatten_all()?, 4)?,
        conv3d_reference(&input, &kernel, (1, 1, 3, 3, 3), (1, 1, 3, 3, 3), 1, 1)
    );
    ...
    let res = Tensor::new(input.clone(), dev)?
        .reshape((1, 1, 5, 5, 5))?
        .conv3d(&Tensor::new(kernel.clone(), dev)?.reshape((1, 1, 3, 3, 3))?, 0, 2, 1, 1)?;
    assert_eq!(res.dims(), &[1, 1, 2, 2, 2]);
    assert_eq!(
        test_utils::to_vec1_round(&res.flatten_all()?, 4)?,
        conv3d_reference(&input, &kernel, (1, 1, 5, 5, 5), (1, 1, 3, 3, 3), 0, 2)
    );
    ...
    let res = Tensor::new(input.clone(), dev)?
        .reshape((2, 2, 3, 3, 3))?
        .conv3d(&Tensor::new(kernel.clone(), dev)?.reshape((2, 2, 2, 2, 2))?, 0, 1, 1, 1)?;
    assert_eq!(res.dims(), &[2, 2, 2, 2, 2]);
    assert_eq!(
        test_utils::to_vec1_round(&res.flatten_all()?, 4)?,
        conv3d_reference(&input, &kernel, (2, 2, 3, 3, 3), (2, 2, 2, 2, 2), 0, 1)
    );
...
test_device!(
    conv3d_plan_cases,
    conv3d_plan_cases_cpu,
    conv3d_plan_cases_gpu,
    conv3d_plan_cases_metal
);
