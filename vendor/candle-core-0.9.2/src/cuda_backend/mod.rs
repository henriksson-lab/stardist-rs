...
impl Im2Col3D {
    ...
    fn dhw_out(&self, d: usize, h: usize, w: usize) -> (usize, usize, usize) {
        let d_out = (d + 2 * self.padding - self.dilation * (self.d_k - 1) - 1) / self.stride + 1;
        let h_out = (h + 2 * self.padding - self.dilation * (self.h_k - 1) - 1) / self.stride + 1;
        let w_out = (w + 2 * self.padding - self.dilation * (self.w_k - 1) - 1) / self.stride + 1;
        (d_out, h_out, w_out)
    }
...
impl Map1 for Im2Col3D {
    fn f<T: DeviceRepr + WithDType>(
        ...
        let dst_el =
            dims[0] * d_out * h_out * w_out * dims[1] * self.d_k * self.h_k * self.w_k;
        let cfg = LaunchConfig::for_num_elems(dst_el as u32);
        let ds = dev.clone_htod(&[dims, layout.stride()].concat())?;
        let src = &src.slice(layout.start_offset()..);
...
impl BackendStorage for CudaStorage {
    ...
    fn conv3d(
        ...
        let device = self.device().clone();
        if !kernel_l.is_contiguous() {
            crate::bail!("conv3d requires a contiguous kernel on cuda")
        }
        let dst_el =
            params.c_out * params.out_d() * params.out_h() * params.out_w() * params.b_size;
        let slice = match (&self.slice, &kernel.slice) {
            (S::U8(inp), S::U8(k)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
